...
module.exports = function serializer(msg){
	...
	function attr(group, name, obj){
      ...
      var syntax;
		if(!groupName) {
          // console.log('data is', group, name, obj);
          // throw "Unknown attribute: " + name
          syntax = {type: 'name', max: 1023};
        }
      else {
        syntax = attributes[groupName][name];
      }

		if(!syntax) {
          console.log('syntax is', attributes, groupName, name);
          throw "Unknown attribute: " + name;
        };
    ...
    if (msg.operation) {
      write2(msg.operation? operations[msg.operation] || msg.operation : statusCodes[msg.statusCode]);
    }
